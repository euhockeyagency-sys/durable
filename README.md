...
`/admin` — кабинет: список заявок игроков и клубных запросов со сменой статуса (`new`/`contacted`/`qualified`/`rejected`/`archived`), поиск и фильтры по статусу, позиции и возрасту, результат калькулятора (со слов игрока). Вход по паролю (`/admin/login`), сессия — подписанная cookie `HttpOnly; SameSite=Strict; Secure` на 12 часов, формы защищены CSRF-токеном, попытки входа ограничены (10 за 15 минут). Задайте `ADMIN_PASSWORD` (от 12 символов, лучше длинный случайный) в окружении, иначе роут отключён; смена пароля завершает все сессии. Старый вход по секретному URL (`ADMIN_SECRET`) удалён. Отдаётся с `X-Robots-Tag: noindex` и `Cache-Control: no-store`, нигде не публикуется. Для каждой заявки есть внутренняя заметка и дата следующего контакта (`applications.internal_note`, `applications.next_contact_at`) и фильтр Follow-up (`due`/`upcoming`/`none`). Без ручной смены статуса заявка так и останется `new` и будет автоматически удалена через 12 месяцев вместе с файлами (см. `supabase/functions/cleanup-applications`); заявка с датой следующего контакта «сегодня или позже» не удаляется. Миграции и функцию из `supabase/` выкладывает интеграция Supabase с GitHub при пуше в `main` (проверка «Supabase Preview» на коммите); шаг «Apply Supabase migrations» в `deploy.yml` пропускается, пока в секретах GitHub нет Supabase-токенов. Порядок относительно SSH-деплоя не гарантирован, поэтому после пуша со схемой убедитесь, что миграция применилась (`supabase migration list` или панель Supabase), прежде чем полагаться на новые колонки.
...
HTML-страницы используют `ETag` и отвечают `304 Not Modified` при повторной проверке. Шрифты и изображения (и версионированные `?v=` CSS/JS) отдаются с `Cache-Control: public, max-age=31536000, immutable` — их содержимое меняется только вместе с именем файла или `?v=`, поэтому браузер не перепроверяет их год. Node также отдаёт Brotli (`br`) или gzip, если клиент это поддерживает. Если перед Node используется Caddy, предпочтительно включить компрессию на нём (`encode zstd br gzip`), чтобы не тратить CPU приложения.
